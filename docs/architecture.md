...
| Redis | Queue backend. |
| Ollama | Optional local inference. Not used in Phase 7. |
...
Phase 7 wires `auth`, `users`, `workspaces`, `repositories`, history, source/symbols, graph, Code DNA/risk insights, deterministic impact, `audit`, and the `repository-sync` worker (clone, Git history, AST parse, graph build, and DNA scoring). Remaining modules stay planned. Controllers stay thin. Business rules live in services and domain packages.

## Data

PostgreSQL tables are listed in the project scope. Phase 7 reuses `graph_edges` and `risk_scores` for on-request blast-radius analysis. Phase 6 already shipped `symbol_versions`, `metrics_snapshots`, and `risk_scores`. `pgvector` is enabled only when semantic embeddings are on.
