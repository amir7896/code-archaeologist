...
function FileList({
  ...
  return (
    <div className="flex-1 overflow-y-auto p-2">
      {filesQuery.isPending ? <p className={`px-2 py-3 ${muted}`}>Loading files…</p> : null}
      {filesQuery.isError ? (
        <p className="px-2 py-3 text-sm text-red-600">{errorMessage(filesQuery.error, 'Unable to load files')}</p>
      ) : null}
      ...
      <ul>
        {files.map((file) => (
          <li key={file.id}>
            <button
              className={`w-full rounded-lg px-3 py-2 text-left ${file.id === selectedId ? 'bg-indigo-50' : 'hover:bg-zinc-50'}`}
              type="button"
              onClick={() => onSelect(file.id)}
            >
              <p className="truncate text-sm font-medium text-zinc-900">{file.path}</p>
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        {previewQuery.data ? (
          <>
            <pre className="mt-3 overflow-x-auto rounded-xl bg-white p-4 text-xs leading-5 text-zinc-800 ring-1 ring-zinc-200">
